...
function novafw_is_woocommerce() {
 ...
 * @return string
 */
if(!( function_exists('novafw_the_terms') )){
	function novafw_the_terms( $cat, $sep, $value, $args = array() ) {
...
/**
 * novafw sanitize title
 * A replacement function for WordPress santize_title which breaks in Russian and other languages.
 * 
 * @since 1.0.0
 * @author tommusrhodus
 */
if(!( function_exists('novafw_sanitize_title') )){
	function novafw_sanitize_title($string){
...
if(!( function_exists('novafw_add_post_thumbnail_column') )){
	function novafw_add_post_thumbnail_column($cols){
	  $cols['novafw_post_thumb'] = __('Featured Image','novafw');
	  return $cols;
	}
}
add_filter('manage_posts_columns', 'novafw_add_post_thumbnail_column', 5);
add_filter('manage_pages_columns', 'novafw_add_post_thumbnail_column', 5);


if(!( function_exists('novafw_display_post_thumbnail_column') )){
	function novafw_display_post_thumbnail_column($col, $id){
	  switch($col){
	    case 'novafw_post_thumb':
...
add_action('manage_posts_custom_column', 'novafw_display_post_thumbnail_column', 5, 2);
add_action('manage_pages_custom_column', 'novafw_display_post_thumbnail_column', 5, 2);
 ...
 * @author tommusrhodus
 */
if(!( function_exists('novafw_hex2rgb') )){
	function novafw_hex2rgb($hex) {
	   $hex = str_replace("#", "", $hex);
 ...
 * @author tommusrhodus
 */
if(!( function_exists('novafw_framework_add_customize_page_link') )){
	function novafw_framework_add_customize_page_link() {
		$theme = wp_get_theme();
		add_dashboard_page( $theme->get( 'Name' ) . ' Theme Options', $theme->get( 'Name' ) . ' Theme Options', 'edit_theme_options', 'customize.php' );
	}
	add_action ('admin_menu', 'novafw_framework_add_customize_page_link');
}

/**
 * Novafw Load Favicons
 * Prints Custom Favicons to wp_head()
 * @since 1.0.0
 * @author DzungNova
 */
if(!( function_exists('novafw_framework_load_favicons') )){
	function novafw_framework_load_favicons() {
	...
	add_action('wp_head', 'novafw_framework_load_favicons');
}
 ...
 * @author _s theme
 */
if(!( function_exists('novafw_framework_wp_title') )){
	function novafw_framework_wp_title( $title, $sep ) {
		global $page, $paged;
		...
		if ( $paged >= 2 || $page >= 2 )
			$title .= " $sep " . sprintf( __( 'Page %s', 'novafw-framework' ), max( $paged, $page ) );
	
		return $title;
	}
	add_filter( 'wp_title', 'novafw_framework_wp_title', 10, 2 );
}
